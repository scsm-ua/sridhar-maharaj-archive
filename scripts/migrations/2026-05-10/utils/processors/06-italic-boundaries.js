...
function isWrappedFullLine(innerText, startDelim, endDelim, match, offset, fullString) {
  const normalizedInner = innerText.replace(/\\$/, '');
  if ((startDelim && !normalizedInner.startsWith(startDelim)) || (endDelim && !normalizedInner.endsWith(endDelim))) {
    return false;
  }
  ...
  // If the match is the entire line content, return true (allow optional trailing \)
  return trimmedLine.replace(/\\$/, '') === match;
}
...
/**
 * Fix italic formatting boundaries on a string
 * @param {string} content - The markdown content
 * @returns {Object} - { content: string, modified: boolean }
 */
function fixItalicBoundariesStr(content) {
  let modified = false;
  
  // Process italic patterns: *text* but not **bold**
  const result = content.replace(/(?<!\*)\*(?!\*)([^*\n]+?)\*(?!\*)/g, (match, innerText, offset, fullString) => {
    // Skip if italic immediately follows a footnote reference marker [^xxx]:
    const lineStart = fullString.lastIndexOf('\n', offset - 1) + 1;
    const textBeforeMatch = fullString.slice(lineStart, offset);
    if (/^\s*\[\^[^\]]+\]:\s*$/.test(textBeforeMatch)) {
      return match;
    }
    ...
    // Also skip any full-line italic regardless of delimiters
    if (isWrappedFullLine(innerText, '(', ')', match, offset, fullString) ||
        ...
        isWrappedFullLine(innerText, '«', null, match, offset, fullString) ||
        isWrappedFullLine(innerText, null, '-', match, offset, fullString) ||
        isWrappedFullLine(innerText, null, null, match, offset, fullString)) {
      return match;
    }
...
/**
 * Fix italic formatting boundaries for a Document
 * @param {Document} doc
 */
function fixItalicBoundaries(doc) {
  const { content, modified } = fixItalicBoundariesStr(doc.text);
  if (modified) {
    doc.text = content;
  }
}
